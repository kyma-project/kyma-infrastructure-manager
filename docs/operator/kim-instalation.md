...
> ### Note:
> For restricted markets, when [Runtime Bootstrapper](../operations-rt-bootstrapper/01-runtime-bootstrapper.md) is enabled (`runtime-bootstrapper-enabled: true`), KIM automatically installs and configures it on each Kyma runtime, and no manual installation or configuration is required.
