...
func TestConfigExplorer_RegistryCacheConfigExists(t *testing.T) {
	...
	t.Run("Return true if at least on RegistryCacheConfig CR exist", func(t *testing.T) {
		// given
		...
		explorer, err := NewConfigExplorer(ctx, client)
		Expect(err).To(BeNil())
	...
	t.Run("Return error when failed to list RegistryCacheConfig CRs", func(t *testing.T) {
		// Test RegistryCacheConfigExists with no registry caches
